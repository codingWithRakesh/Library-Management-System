...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_book_submit'])) {
    ...
    if (move_uploaded_file($imageTmpName, $targetImagePath) && (empty($pdfFileName) || move_uploaded_file($pdfTmpName, $targetPdfPath))) {
        $insertSql = "INSERT INTO books (name, author, category, discription, image_path, pdf_path, isUrl) VALUES ('$name', '$author', '$category', '$description', '$imageFileName', '$targetPdfPath', FALSE)";
...
<body>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            var imageBoxText = document.getElementById("image-box-text");
            var imageInlinePreview = document.getElementById("image-inline-preview");
            var clearImageBtn = document.getElementById("clear-image");

            var pdfInput = document.getElementById("pdf");
            var pdfBoxText = document.getElementById("pdf-box-text");
            var clearPdfBtn = document.getElementById("clear-pdf");
            ...
            function resetImageSelection() {
                ...
                if (imageBoxText) {
                    imageBoxText.textContent = "Click to upload book image";
                }
                if (imageInlinePreview) {
                    imageInlinePreview.style.display = "none";
                    imageInlinePreview.removeAttribute("src");
                }
                if (clearImageBtn) {
            ...
            function resetPdfSelection() {
                ...
                if (pdfBoxText) {
                    pdfBoxText.textContent = "Click to upload book PDF";
                }
                if (clearPdfBtn) {
            ...
            if (imageInput && imageBoxText && imageInlinePreview) {
                imageInput.addEventListener("change", function(event) {
                    ...
                    if (!file.type || !file.type.startsWith("image/")) {
                        resetImageSelection();
                        imageBoxText.textContent = "Invalid image. Click to upload book image";
                        return;
                    }
                    ...
                    imageInlinePreview.src = imageObjectUrl;
                    imageInlinePreview.style.display = "block";
                    imageBoxText.textContent = "";
            ...
            if (pdfInput && pdfBoxText) {
                pdfInput.addEventListener("change", function(event) {
                    ...
                    pdfBoxText.textContent = file.name;
                    if (clearPdfBtn) {
                        clearPdfBtn.style.display = "flex";
                    }
                });
            }
            ...
            if (clearPdfBtn) {
                clearPdfBtn.addEventListener("click", function(event) {
                    event.preventDefault();
                    event.stopPropagation();
                    resetPdfSelection();
                });
